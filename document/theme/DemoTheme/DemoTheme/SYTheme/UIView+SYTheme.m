...
//    [self setBackgroundColor:UIColorFromRGB([[SYTheme shareTheme] getColorWithColorID:colorID])];
}
...
- (void)setDn_isNeedTheme:(BOOL)dn_isNeedTheme {
    ...
    if (dn_isNeedTheme && !self.dn_isHasRegistNotif) {
        //未注测过通知且需要主题通知，进行注册
//        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(dn_onDayNightStateChange:) name:GNDAYNIGHT_STATE_CHANGE object:nil];
    } else if(!dn_isNeedTheme && self.dn_isHasRegistNotif){
        //设置为不需要注册，且已经注册，则把通知移除
...
//        }];
    ...
    if ([self dn_themeChangeBlock]) {
        ...
        block([[SYTheme shareTheme] themeStyle]);
    }
    
    //子类可以复写此方法
    [self dn_onDayNightStateHasChange:[[SYTheme shareTheme] themeStyle]];
}
